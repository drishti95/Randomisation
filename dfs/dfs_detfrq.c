...
igraph_bool_t dfs_callback(const igraph_t *graph,
         igraph_integer_t vid,igraph_integer_t dist,void *extra) {
...
for(i=0;i<n;i++)
{
igraph_dfsd(&graph, /*root=*/(igraph_integer_t)i, /*neimode=*/ IGRAPH_ALL, 
...
//fp=fopen("abc.txt","a+");
  ...
  for(i=0;i<10;i++)
{
 igraph_dfsd(&graph, /*root=*/0, /*neimode=*/ IGRAPH_ALL, 
   /*unreachable=*/ 1,&order,0,0,0, 
  /*in_callback=*/&dfs_callback,/*out_callback*/&dfs_callback, /*extra=*/ 0);

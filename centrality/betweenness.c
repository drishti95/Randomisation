...
int igraph_betweenness_estimate(const igraph_t *graph, igraph_vector_t *res, 
  ...
  for (source=0; source<no_of_nodes; source++) {
    ...
    while (!igraph_stack_empty(&stack)) {
      ...
      printf(" STCAK ACTNODE %li : ", actnode);
	...
	igraph_biguint_set_limb(&big_nrgeo[actnode], 0);
        ...
        printf("tmpscore at actnode %lf",tmpscore[actnode]);
      tmpscore[actnode]=0;
      igraph_vector_int_clear(igraph_adjlist_get(adjlist_in_p, actnode));
    }
  
 printf("tmpres");
  igraph_vector_print(tmpres);
...
int main() {
  ...
  printf("Ring\n");
 // igraph_betweenness_estimate(&g,&res,vs,IGRAPH_OUT,(igraph_real_t)n,
//				     NULL,1);
